...
impl Dfa {
    ...
    /// Given a state, returns whether to consume input before its transition.
    pub fn consumes(&self, state: u16) -> bool {
        let transition = &self.transitions[state as usize];
        transition.consume
    }

    /// Given a state and an input symbol,
...
impl Nfa {
    ...
    /// Given a state, returns whether input is consumed before its transition.
    pub fn consumes(&self, state: u16) -> bool {
        let transition = &self.transitions[state as usize];
        transition.consume
    }
    ...
    pub fn run<I: Iterator<Item = u8>>(&self, input: I) -> bool {
        let mut states_a = HashSet::new();
        let mut states_b = HashSet::new();
        let mut states_c = HashSet::new();
        ...
        for symbol in input {
            ...
            loop {
                let state = *match states_a.iter().next() {
                    Some(x) => x,
                    None => break,
                };
                states_a.remove(&state);
